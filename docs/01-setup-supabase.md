...
Supabase ist gratis bis 500 MB DB + 5 GB Egress/Monat. Reicht fuer mehrere Millionen Events.
...
2. Region: am besten EU (Frankfurt) — geringere Latenz fuer deutsche Besucher
3. DB-Passwort generieren + sicher speichern
4. ~2 Min warten bis das Projekt provisioniert ist

## 2. Tabelle anlegen

Im Supabase Dashboard → "SQL Editor" → "New query" → das hier einfuegen + Run:

```sql
-- Tabelle fuer alle Tracking-Events
create table public.link_events (
    id          bigint generated always as identity primary key,
    event_type  text not null check (event_type in ('view', 'click')),
    link_name   text,                       -- nur fuer event_type='click'
...
-- Policy 2: NICHTS sonst fuer anon — kein SELECT, kein UPDATE, kein DELETE.
...
3. Supabase Dashboard → "Table editor" → `link_events` → es sollten 2 Eintraege da sein (1x `view`, 1x `click`)
